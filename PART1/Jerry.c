...
{
	if(PName == NULL || X<0 || Y<0 || Z<0)//Sanity test (checks that the values obtained make sense)
		return NULL;

	planet *Myplanet = (planet*)malloc(sizeof(planet)); // Allocating memory for planet
	if(Myplanet == NULL)
		{
			printf("Memory Problem");//Check that the memory has been assigned
			return NULL;
		}
...
status AddPhysicalCharacteristics(Jerry* MyJerry, char *PhysicalName,float PhysicalVal)//Adds PhysicalInfo to a Jerry
{
	...
	{
		MyJerry->PhysicalInfo= (PhysicalInfo**)realloc(MyJerry->PhysicalInfo,sizeof(MyJerry->PhysicalInfo+1)* sizeof(PhysicalInfo*));//Resize the existing list
		if (MyJerry->PhysicalInfo == NULL) // Checks if the allocation succeed
		{
					return NoMemory;
		}
		MyJerry->PhysicalInfo[MyJerry->numOfPhysicalInfo] = CreatePhysicalCharacteristics(PhysicalName,PhysicalVal);
		if (MyJerry->PhysicalInfo[MyJerry->numOfPhysicalInfo] == NULL) // Checks if we Insert the type to the designated index
		{
			return failure;
		}
